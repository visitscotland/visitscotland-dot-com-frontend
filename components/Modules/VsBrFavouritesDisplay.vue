            ...
            <VsCol>
                <div v-if="uiState === 'error'">
                    <div class="d-flex justify-content-center">
                        <div
                            class="d-flex flex-column gap-200 align-items-center"
                ...
                <div v-else-if="uiState === 'loading'">
                    <!-- Flash of content was flagged as an issue in QA, but Loading state has not
                    been designed for.    -->
                </div>
...
const uiState = computed(() => {
    if (fetchRequestStatus.value === 'error') {
        return 'error';
    }
    ...
    if (favourites.pages.length === 0 || cardData.value.length === 0) {
        return 'empty';
    }
    return 'ready';
});
...
async function getCollectionList(shareId) {
    try {
        ...
        await getCollectionData(favouritesEndpoint, uuids);
    } catch (err) {
